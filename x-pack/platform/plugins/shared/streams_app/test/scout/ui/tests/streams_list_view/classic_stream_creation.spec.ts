...
function getCreatedStreamNameFromUrl(urlString: string): string {
  const match = urlString.match(/\/app\/streams\/([^/]+)\/management\/overview/);
  ...
  () => {
    test('creates a classic stream via the creation flyout', async ({
      ...
      try {
        ...
        // Wait for navigation to the created stream management page (Overview tab).
        await expect(page).toHaveURL(/\/app\/streams\/.+\/management\/overview/);
